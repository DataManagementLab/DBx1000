...
txn_man::validate_tictoc()
{
	...
	//! XXX: Claimed to be very expensive!
	if (!BOUNDED_SPINNING || !_validation_no_wait)
	{
	...
	if (_pre_abort) {
		for (int i = 0; i < wr_cnt; i++) {
			...
			if (row->getManager()->get_wts() != accesses[write_set[i]]->wts)
			{
				rc = Abort;
				goto final;
			}
		...
		for (int i = 0; i < row_cnt - wr_cnt; i++) {
			...
			bool lock;
			uint64_t wts, rts;
			row->getManager()->get_ts_word(lock, rts, wts);
#if TICTOC_MV 
		...
		while (!done) {
			...
			if (num_locks == wr_cnt)
				...
				for (int i = 0; i < num_locks; i++)
					accesses[write_set[i]]->orig_row->getManager()->release();
				if (_pre_abort) {
					num_locks = 0;
					for (int i = 0; i < wr_cnt; i++) {
						row_t* row = accesses[write_set[i]]->orig_row;
					...
					for (int i = 0; i < row_cnt - wr_cnt; i++) {
						...
						bool lock;
						uint64_t wts, rts;
						access->orig_row->getManager()->get_ts_word(lock, rts, wts);
#if TICTOC_MV 
	...
	for (int i = 0; i < wr_cnt; i++) {
		row_t* row = accesses[write_set[i]]->orig_row;
		if (row->getManager()->get_rts() + 1 > commit_wts)
			commit_wts = row->getManager()->get_rts() + 1;
	}
	...
	for (int i = 0; i < row_cnt - wr_cnt; i++) {
		...
		if (access->rts < commit_wts) {
			bool success = access->orig_row->getManager()->try_renew(access->wts, commit_wts, access->rts, get_thd_id());
#elif ISOLATION_LEVEL == SNAPSHOT
		Access* access = accesses[read_set[i]];
		if (access->rts < commit_rts) {
			bool success = access->orig_row->getManager()->try_renew(access->wts, commit_rts, access->rts, get_thd_id());
#endif
			if (!success) {
				rc = Abort;
				goto final;
	...
	/*	for (int i = 0; i < row_cnt; i++) {
			...
			num_locks++;
			if (accesses[ rid ]->type == WR) {
				if (row->getManager()->get_wts() != accesses[ rid ]->wts) {
					rc = Abort;
					goto final;
				}
				if (row->getManager()->get_rts() + 1 > max_wts)
					max_wts = row->getManager()->get_rts() + 1;
			} else if (accesses[rid]->type == RD) {
				if (row->getManager()->get_wts() != accesses[rid]->wts
						&& max_wts > row->getManager()->get_wts())
				{
					rc = Abort;
					goto final;
				}
		...
		for (int i = 0; i < row_cnt - wr_cnt; i++) {
			Access * access = accesses[ read_set[i] ];
			if (!access->orig_row->getManager()->renew_lease(access->wts, access->rts))
			{
				rc = Abort;
				goto final;
			}
	...
	if (rc == Abort) {
...
#else 
		for (int i = 0; i < num_locks; i++)
			accesses[sorted_set[i]]->orig_row->getManager()->release();
#endif
		cleanup(rc);
	}
	else {
		...
		if (_write_copy_ptr) {
			...
			for (int i = 0; i < wr_cnt; i++) {
				...
				access->orig_row->getManager()->write_data(
					access->data, commit_wts);
				access->orig_row->getManager()->release();
			}
#else 
			//			for (int i = 0; i < row_cnt; i++) {
			//				Access * access = accesses[ i ];
			//				if (access->type == WR)
			//					access->orig_row->getManager()->write_data(access->data, max_wts);
			//				access->orig_row->getManager()->release();
			//			}
#endif
		}
