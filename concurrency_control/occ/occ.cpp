...
OptCC::per_row_validate(txn_man* txn) {
	...
	int lock_cnt = 0;
#if SCOPED_LATCH
	SCOPED_LATCH_T::scoped_lock* latches = new SCOPED_LATCH_T::scoped_lock[txn->row_cnt];
#define CUR_LATCH latches[i]
#else
#define CUR_LATCH
#endif
...
#if BOUNDED_SPINNING
		// Try to acquire latch including back-off
		// Assume deadlock if latch cannot be taken	
		ok = txn->accesses[i]->orig_row->getManager()->try_latch(CUR_LATCH);
		if (ok) // Validate record if latch is taken
		{
			lock_cnt++;
			ok = txn->accesses[i]->orig_row->getManager()->validate(txn->start_ts);
		}
#else
		lock_cnt++;
		txn->accesses[i]->orig_row->getManager()->latch(CUR_LATCH);
		ok = txn->accesses[i]->orig_row->getManager()->validate(txn->start_ts);
#endif  // BOUNDED_SPINNING
	}
	if (ok) {
	...
	for (int i = 0; i < lock_cnt; i++)
		txn->accesses[i]->orig_row->getManager()->release();
#endif  // SCOPED_LATCH
#undef CUR_LATCH
#endif
	return rc;
