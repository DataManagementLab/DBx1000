...
void Row_lock::init(row_t* row) {
	...
	//latch = new SCOPED_LATCH_T;
#endif
	new (&latch) SCOPED_LATCH_T;
...
RC Row_lock::lock_get(lock_t type, txn_man* txn, uint64_t*& txnids, int& txncnt) {
	...
	if (conflict) {
		...
		if (CC_ALG == NO_WAIT) {
			...
			waiter_cnt++;
			txn->lock_ready.store(false, std::memory_order_relaxed);
			rc = WAIT;
		}
...
RC Row_lock::lock_release(txn_man* txn) {
	...
	while (waiters_head && !conflict_lock(lock_type, waiters_head->type)) {
		...
		entry->txn->lock_ready.store(true, std::memory_order_relaxed);
