...
RC Row_hekaton::access(txn_man* txn, TsType type, row_t* row) {
	...
	backoff_function bf;
	ts_t ts = txn->get_ts();
	while(!blatch.compare_exchange_weak(false, true, std::memory_order_relaxed))
		bf();
	bf.reset();
	...
	if (type == R_REQ) {
		if (ISOLATION_LEVEL == REPEATABLE_READ) {
			rc = RCOK;
			txn->cur_row.store(_write_history[_his_latest].row, std::memory_order_relaxed);
		}
			...
			while (true) {
				...
				if (_write_history[i].begin < ts) {
					...
					txn->cur_row(_write_history[i].row, std::memory_order_relaxed);
					find = true;
					break;
				}
				if (i == _his_oldest)
