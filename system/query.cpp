...
Query_thd::init(workload * h_wl, int thread_id) {
	...
	size_t size_part_to_access = sizeof(uint64_t) * g_part_cnt;
	auto pre_alloced_parts_to_access = (uint64_t*)_mm_malloc( size_part_to_access * request_cnt, 64);
#endif
	for (UInt32 qid = 0; qid < request_cnt; qid++) {
		...
		queries[qid].init(thread_id, h_wl, pre_alloced_parts_to_access + g_part_cnt * qid);
		if(qid > 0) {
			if(queries[qid].part_to_access == queries[qid -1].part_to_access) {
				std::abort();
			}
		}
#endif
