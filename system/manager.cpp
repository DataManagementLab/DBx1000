...
void Manager::init() {
	...
	*timestamp = 1;
	_last_min_ts_time = 0;
	_min_ts = 0;
	_epoch = (std::atomic_uint64_t*)_mm_malloc(sizeof(uint64_t), 64);
	_last_epoch_update_time = (ts_t*)_mm_malloc(sizeof(uint64_t), 64);
	_epoch = 0;
	_last_epoch_update_time = 0;
	all_ts = (std::atomic<ts_t>**)_mm_malloc(sizeof(std::atomic<ts_t>*) * g_thread_cnt, 64);
	for (uint32_t i = 0; i < g_thread_cnt; i++)
		all_ts[i] = (std::atomic<ts_t>*)_mm_malloc(sizeof(ts_t), 64);
...
ts_t Manager::get_min_ts(uint64_t tid) {
	...
	// Utilizing load from _last_min_ts_time as fence for below reads
	uint64_t last_time = _last_min_ts_time.load(std::memory_order_acquire);
	if (tid == 0 && now - last_time > MIN_TS_INTVL)
	{
		ts_t min = UINT64_MAX;
		for (UInt32 i = 0; i < g_thread_cnt; i++) {
			auto ts = all_ts[i]->load(std::memory_order_relaxed);
			if (ts < min)
				min = ts;
		}
		if (min > _min_ts)
...
void Manager::add_ts(uint64_t thd_id, ts_t ts) {
	...
	all_ts[thd_id]->store(ts, std::memory_order_acq_rel);
}
