...
public:
	...
	// sets tablename and stores schema
	...
	RC get_new_row(row_t*& row, uint64_t part_id, uint64_t& row_id, uint64_t tuple_size);
	RC get_new_row(row_t*& row, uint64_t part_id, uint64_t& row_id, uint64_t tuple_size, uint64_t count);
...
private:
	...
	char 			pad[CL_SIZE - sizeof(void*) * 2 - sizeof(uint64_t)];
};
